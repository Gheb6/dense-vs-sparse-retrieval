...
\begin{frame}{Operational Advice: RQ1 \& RQ2}
    ...
    \begin{table}
        ...
        \begin{tabular}{l l c c c c l} 
            \toprule
            \textbf{Dataset} & \textbf{Method} & \textbf{Build} & \textbf{RAM} & \textbf{QPS} & \textbf{nDCG} & \textbf{Verdict / Trade-off} \\
            \midrule
            
            % SCIFACT
            \textbf{SciFact} 
            & Flat (FP32) & \textbf{$<$0.1s} & .01 GB & \textbf{4,205} & \textbf{0.738} & \textcolor{green!60!black}{\textbf{Best Choice}} \\
            \textit{(5k)}
            & Flat (INT8) & $<$0.1s & .01 GB & 2,209 & 0.736 & \textcolor{red!80!black}{\textit{Slower (CPU limit)}} \\
            \cmidrule(l){2-7}
            
            % QUORA
            \textbf{Quora} 
            & Flat (FP32) & \textbf{1s} & .38 GB & 120 & 0.889 & \textit{Too Slow} \\
            \textit{(523k)}
            & HNSW (FP32) & 4m & .45 GB & \textbf{361} & 0.889 & \textcolor{blue}{\textbf{Worth the wait}} \\
            \cmidrule(l){2-7}
            
            % NQ
            \textbf{NQ} 
            & HNSW (FP32) & 19m & 2.4 GB & 9,034 & 0.464 & \textit{High RAM} \\
            \textit{(2.6M)}
            & HNSW (\textbf{INT8}) & 25m & \textbf{0.6 GB} & \textbf{9,100} & 0.468 & \textcolor{blue}{\textbf{-75\% RAM (Safe)}} \\
    ...
    \begin{block}{Final Recommendations}
        \scriptsize 
        \begin{itemize}
            \setlength\itemsep{0.2em}
            \item \textbf{RQ1 (Scale):} Use \textbf{Flat} for small data (<100k). Use \textbf{HNSW} for large data (>200k) as QPS scales better, with negligible nDCG drop (-0.004).
            \item \textbf{RQ2 (Quantization):} Always use \textbf{INT8} for HNSW. It saves 75\% RAM while maintaining identical nDCG quality (0.541 vs 0.540).
        \end{itemize}
    \end{block}
